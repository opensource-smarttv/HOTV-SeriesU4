/******************************************************************
*  $Id: nanohttp-ssl.c 482792 2015-03-13 12:42:29Z stahlan $
*
...
_hssl_server_context_init(void)
{
  ...
  SSL_CTX_set_verify(context, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT | SSL_VERIFY_CLIENT_ONCE,
    ...
    if ((count = SSL_read(sock->ssl, buf, len)) < 1)
      return herror_new("SSL_read", HSOCKET_ERROR_RECEIVE,
                        "SSL_read failed (%s)", _hssl_get_error(sock->ssl,
                                                                count));
      // Timo Weggen: Just in case, SSL doesn't handle this.
      if (0==count) {
      errno = ENOENT;
      return herror_new("hssp_read", HSOCKET_ERROR_SSLCLOSE,
  ...
  else
  {
    if ((count = hsocket_select_read(sock->sock, buf, len)) == -1)
      return herror_new("hssl_read", HSOCKET_ERROR_RECEIVE,
                        "recv failed (%s)", strerror(errno));
    // Timo Weggen: Added EOF detection.
    if (0==count) {
      errno = ENOENT;
      return herror_new("hssp_read", HSOCKET_ERROR_SSLCLOSE,
                        "recv failed (%s)", strerror(errno));
