/******************************************************************
*  $Id: nanohttp-server.c 141329 2012-01-24 09:27:11Z kutterol $
*
...
// max. number of similar (i.e. from the same ip) connections
#define HTTPD_MAX_NUM_OF_SIMILARY_CONNECTIONS 2
...
static hsocket_t _httpd_unencrypted_socket;
static int _httpd_port = 10000;
static int _httpd_unencrypted_port = 10001;
...
_httpd_parse_arguments(int argc, char **argv)
{
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    else if (!strcmp(argv[i - 1], NHTTPD_ARG_UNENCRYPTED_PORT))
    {
      _httpd_unencrypted_port = atoi(argv[i]);
    }
  ...
  log_verbose2("unencrypted socket bind to port '%d'", _httpd_unencrypted_port);
...
httpd_init(int argc, char *argv[])
{
  ...
  if ((status = hsocket_init(&_httpd_unencrypted_socket)) != H_OK)
  {
    log_error2("hsocket_init (secure) failed (%s)", herror_message(status));
    return status;
  }

  {
      herror_t rc = hsocket_bind(&_httpd_socket, _httpd_port);
      ...
      rc = hsocket_bind(&_httpd_unencrypted_socket, _httpd_unencrypted_port);
      return rc;
  }
}
...
#endif
{
  ...
  while (!done)
  {
    ...
    else
    {
      ...
      if (!done) {
#ifdef    ENABLE_TEMPORARY_IPAD_WORKAROUND
        done=1;
#else  // ENABLE_TEMPORARY_IPAD_WORKAROUND
        done = req->version == HTTP_1_0 ? 1 : 0;
#endif // ENABLE_TEMPORARY_IPAD_WORKAROUND
      }
...
int httpd_get_number_of_similar_connections( conndata_t *conn )
{
  int i, res = 0;
  ...
  if( !_httpd_run )
  {
    ...
    for( i=0; i<_httpd_max_connections; i++ )
    {
        if( _httpd_connection[i].flag == CONNECTION_IN_USE )
        {
            if( ( conn != &_httpd_connection[i] ) &&
                !memcmp( &_httpd_connection[i].sock.addr.sin_addr, &conn->sock.addr.sin_addr, sizeof( conn->sock.addr.sin_addr ) ) )
            {
                res++;
            }
        }
...
httpd_run(void)
{
  ...
  if ((err = hsocket_listen(&_httpd_unencrypted_socket)) != H_OK)
  {
    log_error2("hsocket_listen (secure) failed (%s)", herror_message(err));
    return err;
  }
  ...
  while (_httpd_run)
  {
    ...
    while (_httpd_run)
    {
      ...
      FD_SET(_httpd_unencrypted_socket.sock, &fds);

      /* select socket descriptor */

      maxfd = _httpd_socket.sock > _httpd_unencrypted_socket.sock ?
            ( _httpd_socket.sock + 1 ) : ( _httpd_unencrypted_socket.sock + 1 );
      ...
      if ( FD_ISSET(_httpd_socket.sock, &fds) || FD_ISSET(_httpd_unencrypted_socket.sock, &fds) )
      {
        break;
      }
    }
    ...
    {
        log_verbose1("accepting on default socket\n");
        if ((err = hsocket_accept(&_httpd_socket, &(conn->sock),1)) != H_OK)
        {
    ...
    else if( FD_ISSET(_httpd_unencrypted_socket.sock, &fds) )
    {
        log_verbose1("accepting on unencrypted socket\n");
        if ((err = hsocket_accept(&_httpd_unencrypted_socket, &(conn->sock),0)) != H_OK)
        {
          log_error2("hsocket_accept failed (%s)", herror_message(err));
...
#ifdef    ENABLE_TEMPORARY_IPAD_WORKAROUND
    log_error2("ENABLE_TEMPORARY_IPAD_WORKAROUND in %s()\n",__func__);
#else  // ENABLE_TEMPORARY_IPAD_WORKAROUND
    if( httpd_get_number_of_similar_connections( conn ) >= HTTPD_MAX_NUM_OF_SIMILARY_CONNECTIONS )
    {
        log_verbose2("Number of similar connections exceeded. Discarding (one) connection from %s\n",
            SAVE_STR(((char *) inet_ntoa(conn->sock.addr.sin_addr))) );
