/******************************************************************
 *  $Id: soap-ctx.c 141329 2012-01-24 09:27:11Z kutterol $
 *
...
soap_ctx_new(SoapEnv * env)     /* should only be used internally */
{
  ...
  ctx->action = NULL;
  ctx->isSecureSource = 0;

  return ctx;
}

void
soap_ctx_set_secure_source(SoapCtx *ctx)
{
  if( ctx )
  {
    ctx->isSecureSource = 1;
  }
}

int
soap_ctx_is_secure_source(SoapCtx *ctx)
{
  return ctx ? ctx->isSecureSource : 0;
}
