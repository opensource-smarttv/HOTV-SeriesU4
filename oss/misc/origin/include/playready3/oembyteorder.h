...
** Use assignment plus the following macro instead.
** Example: To start with "DRM_UINT64 qw", end with "DRM_UINT64 qwBE", and call "ThisFunctionExpectsBigEndianBytes( (DRM_BYTE*)&qwBE )", do the following:
** DRM_UINT64 qwBE = qw;
** FORMAT_QWORD_AS_BIG_ENDIAN_BYTES_INPLACE( qwBE );
** ChkDR( ThisFunctionExpectsBigEndianBytes( &qwBE ) );
*/
#define FORMAT_QWORD_AS_BIG_ENDIAN_BYTES_INPLACE(qw) do { (qw) = FLIP_QWORD_BYTES(qw); } while( FALSE )
...
/*
** Note: Do *NOT* define MAKE_QWORD_BIG_ENDIAN.  Refer to comments on earlier definition of FORMAT_QWORD_AS_BIG_ENDIAN_BYTES_INPLACE.
*/
#define FORMAT_QWORD_AS_BIG_ENDIAN_BYTES_INPLACE(qw) do { NATIVE64_TO_NONNATIVE64((qw)); } while( FALSE )
