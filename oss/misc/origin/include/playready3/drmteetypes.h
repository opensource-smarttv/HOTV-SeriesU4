...
typedef enum
{
    ...
    DRM_TEE_BYTE_BLOB_TYPE_MSTART_SECURE_MODE = 5
...
typedef struct __tagDRM_TEE_POLICY_RESTRICTION
{
    ...
    */
    ...
    /* Binary Configuration data (size and raw bytes) - note that this is often in BIG ENDIAN and must be converted (see XMR spec for details) */
...
typedef struct __tagDRM_TEE_POLICY_INFO
{
    DRM_KID                     oKID;
    DRM_LID                     oLID;
    DRM_DWORD                   dwDecryptionMode;                                       /* One of the values from the OEM_TEE_AES128CTR_DECRYPTION_MODE enum */
...
typedef enum
{
    DRM_TEE_POLICY_INFO_CALLING_API_AES128CTR_PREPARETODECRYPT              = 1,
