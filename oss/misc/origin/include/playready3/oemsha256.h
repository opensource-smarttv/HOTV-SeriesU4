...
/* Equivalent to OEM_SHA256_Finalize, but the final hash is only OEM_SHA1_DIGEST_LEN (20) instead of OEM_SHA256_DIGEST_SIZE_IN_BYTES (32) */
DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL OEM_SHA256_Finalize_With_SHA_1_Size(
    __inout_ecount( 1 )                 OEM_SHA256_CONTEXT  *f_pContext,
    __out_bcount( OEM_SHA1_DIGEST_LEN ) DRM_BYTE             f_rgbDigest[OEM_SHA1_DIGEST_LEN] ) DRM_NO_INLINE_ATTRIBUTE;


DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL OEM_SHA256_HMAC_Init(
    __inout_ecount( 1 )          OEM_SHA256_HMAC_CONTEXT       *f_pContextHMAC,
    __in_bcount( f_cbKey ) const DRM_BYTE                      *f_pbKey,
    __in                         DRM_DWORD                      f_cbKey ) DRM_NO_INLINE_ATTRIBUTE;

DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL OEM_SHA256_HMAC_Update(
    __inout_ecount( 1 )           OEM_SHA256_HMAC_CONTEXT  *f_pContextHMAC,
    __in_bcount( f_cbData ) const DRM_BYTE                 *f_pbData,
    __in                          DRM_DWORD                 f_cbData ) DRM_NO_INLINE_ATTRIBUTE;

DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL OEM_SHA256_HMAC_Finalize(
    __inout_ecount( 1 )           OEM_SHA256_HMAC_CONTEXT  *f_pContextHMAC,
    __out_bcount( f_cbKeyedHash ) DRM_BYTE                 *f_pbKeyedHash,
    __in                          DRM_DWORD                 f_cbKeyedHash ) DRM_NO_INLINE_ATTRIBUTE;

DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL OEM_SHA256_HMAC_FinalizeOffset(
    __inout_ecount( 1 )                                     OEM_SHA256_HMAC_CONTEXT  *f_pContextHMAC,
...
DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL OEM_SHA256_HMAC_CreateMAC(
    __in_bcount( f_cbKey )                       const DRM_BYTE                      *f_pbKey,
...
DRM_API DRM_RESULT DRM_CALL OEM_SHA256_HMAC_VerifyMAC(
    __in_bcount( f_cbKey )                       const DRM_BYTE                      *f_pbKey,
    __in                                               DRM_DWORD                      f_cbKey,
    __in_bcount( f_cbDataRemaining + f_ibData )  const DRM_BYTE                      *f_pbData,
    __in                                               DRM_DWORD                      f_ibData,
