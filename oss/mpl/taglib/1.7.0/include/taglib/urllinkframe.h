...
namespace TagLib {
  ...
  namespace ID3v2 {
    ...
    public:
       ...
       */
      explicit UrlLinkFrame(const ByteVector &data, uint frameDataPosition);
    ...
    public:
       ...
       */
      explicit UserUrlLinkFrame(uint frameDataPosition, String::Type encoding = String::Latin1);

      /*!
       * This is a dual purpose constructor.  \a data can either be binary data
       * that should be parsed or (at a minimum) the frame ID.
       */
      explicit UserUrlLinkFrame(const ByteVector &data, uint frameDataPosition);
