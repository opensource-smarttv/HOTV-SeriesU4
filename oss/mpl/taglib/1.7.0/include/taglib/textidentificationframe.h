...
namespace TagLib {
  ...
  namespace ID3v2 {
    ...
    public:
       ...
       */
      TextIdentificationFrame(const ByteVector &type, uint frameDataPosition, String::Type encoding);

      /*!
       * This is a dual purpose constructor.  \a data can either be binary data
       * that should be parsed or (at a minimum) the frame ID.
       */
      explicit TextIdentificationFrame(const ByteVector &data, uint frameDataPosition);
    ...
    public:
       ...
       */
      explicit UserTextIdentificationFrame(uint frameDataPosition, String::Type encoding = String::Latin1);

      /*!
       * Creates a frame based on \a data.
       */
      explicit UserTextIdentificationFrame(const ByteVector &data, uint frameDataPosition);
